...
public class Table<K, V> extends AbstractMap<K, V> {
    ...
    public V remove(Object key) {
        int ix = index(key);
        if (ix >= 0) {
            Map.Entry<K,V> removed = table_.remove(ix);
    ...
    public static void main(String[] args) {
        var caps = new Table<String, String>();
        caps.put("Japan", "Edo");    // entry addition
        caps.put("UK", "London");
        caps.put("Soviet", "Moscow");
        System.out.println(caps);
        caps.put("Japan", "Tokyo");  // entry replacement
        System.out.println(caps);
    }
}
